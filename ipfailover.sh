...
function test_offset(){
    ...
    echo -e "$BBlue Check the value in keepalived.conf"
    oc exec `oc get po --config admin.kubeconfig | grep ipf-red | cut -d " " -f1` -- grep -i id /etc/keepalived/keepalived.conf 
    oc exec `oc get po --config admin.kubeconfig | grep ipf-blue | cut -d " " -f1` -- grep -i id /etc/keepalived/keepalived.conf
